...
class LigaPeticeEasyFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_ligapetice_easy, container, false)
    }
    ...
    private fun getQuestion(questionNumber: Int) {
        pitanjaRef.child("Liga petice").child("Laka kategorija").child("$questionNumber")
            ...
            try {
                ...
                builder.setPositiveButton("Sljedeće pitanje") { dialog, which ->
                    ...
                    if (brojTocnihOdgovora == 3) {
                        fragmentManager?.beginTransaction()?.replace(R.id.fragmentContainer, LigaPeticeMediumFragment())?.commit()
                    } else {
                        resetirajIgru()
                        pokreniIgru()
                    }
