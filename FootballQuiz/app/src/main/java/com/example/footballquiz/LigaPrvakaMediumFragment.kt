...
class LigaPrvakaMediumFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_ligaprvaka_medium, container, false)
    }
            ...
            query.addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        val prethodniHighScore = snapshot.child("liga_prvaka_highScore").value as? Long
    ...
    private fun getQuestion(questionNumber: Int) {
        pitanjaRef.child("Liga prvaka").child("Srednje teška kategorija").child("$questionNumber")
        ...
        if (!isFragmentDestroyed) {
            try {
                ...
                builder.setPositiveButton("Sljedeće pitanje") { dialog, which ->
                    ...
                    brojPitanja++
                    if(brojPitanja > 3){
                        fragmentManager?.beginTransaction()?.replace(R.id.fragmentContainer, LigaPrvakaHighFragment())?.commit()
                    }
                    else {
                        resetirajIgru()
                        pokreniIgru()
