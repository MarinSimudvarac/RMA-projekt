...
class LigaPeticeMediumFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_ligapetice_medium, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        iscitajHighScoreIzBaze()
    ...
    private fun iscitajHighScoreIzBaze(){
            ...
            query.addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        val prethodniHighScore = snapshot.child("liga_petice_highScore").value as? Long

                        highScore = prethodniHighScore?.toInt() ?: 0
    ...
    private fun getQuestion(questionNumber: Int) {
        pitanjaRef.child("Liga petice").child("Srednje teška kategorija").child("$questionNumber")
    ...
    private fun provjeriOdgovor(odabraniButton: Button, question: Question) {
        ...
        bodovi = when {
            odabraniButton.text == question.correctAnswer && preostaloVrijeme > 0 -> {
                ...
                when {
                    preostaloVrijemeSec >= 8 -> 1000  // bodovi ako je preostalo vrijeme između 8 i 10 sekundi
                    preostaloVrijemeSec >= 6 -> 800  // bodovi ako je preostalo vrijeme između 6 i 8 sekundi
                    preostaloVrijemeSec >= 4 -> 600  // bodovi ako je preostalo vrijeme između 4 i 6 sekundi
                    preostaloVrijemeSec >= 2 -> 400  // bodovi ako je preostalo vrijeme između 2 i 4 sekunde
                    else -> 200  // bodovi ako je preostalo vrijeme između 1 i 2 sekunde
                }
            }
        ...
        if (!isFragmentDestroyed) {
            try {
                ...
                builder.setPositiveButton("Sljedeće pitanje") { dialog, which ->
                    ...
                    brojPitanja++
                    if(brojPitanja > 3){
                        fragmentManager?.beginTransaction()?.replace(R.id.fragmentContainer, LigaPeticeHighFragment())?.commit()
                    }
                    else {
                        resetirajIgru()
                        pokreniIgru()
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        ...
                        val user = mapOf<String, Any>(
                            "liga_petice_score" to score,
                            "liga_petice_highScore" to highScore
                        )
