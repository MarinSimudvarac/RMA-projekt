...
class SpiEpHighFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_spiep_high, container, false)
    }
            ...
            query.addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        val trenutniScore = snapshot.child("spiep_score").value as? Long
                        val trenutniHighScore = snapshot.child("spiep_highScore").value as? Long
    ...
    private fun getQuestion(questionNumber: Int) {
        pitanjaRef.child("SP i EP").child("Teška kategorija").child("$questionNumber")
        ...
        if (!isFragmentDestroyed) {
            try {
                ...
                builder.setPositiveButton("Sljedeće pitanje") { dialog, which ->
                    ...
                    brojPitanja++
                    if(brojPitanja > 3){
                        fragmentManager?.beginTransaction()?.replace(R.id.fragmentContainer, SpiEpHighscoreFragment())?.commit()
                    }
                    else {
                        resetirajIgru()
                        pokreniIgru()
