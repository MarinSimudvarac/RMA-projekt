...
class SpiEpMediumFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_spiep_medium, container, false)
    }
            ...
            query.addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        val prethodniHighScore = snapshot.child("spiep_highScore").value as? Long
    ...
    private fun getQuestion(questionNumber: Int) {
        pitanjaRef.child("SP i EP").child("Srednje teška kategorija").child("$questionNumber")
        ...
        if (!isFragmentDestroyed) {
            try {
                ...
                builder.setPositiveButton("Sljedeće pitanje") { dialog, which ->
                    ...
                    brojPitanja++
                    if(brojPitanja > 3){
                        fragmentManager?.beginTransaction()?.replace(R.id.fragmentContainer, SpiEpHighFragment())?.commit()
                    }
                    else {
                        resetirajIgru()
                        pokreniIgru()
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        ...
                        val user = mapOf<String, Any>(
                            "spiep_score" to score,
                            "spiep_highScore" to highScore
                        )
