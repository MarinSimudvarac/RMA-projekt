...
class SpiEpEasyFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_spiep_easy, container, false)
    }
    ...
    private fun getQuestion(questionNumber: Int) {
        pitanjaRef.child("SP i EP").child("Laka kategorija").child("$questionNumber")
            ...
            try {
                ...
                builder.setPositiveButton("Sljedeće pitanje") { dialog, which ->
                    ...
                    if (brojTocnihOdgovora == 3) {
                        fragmentManager?.beginTransaction()?.replace(R.id.fragmentContainer, SpiEpMediumFragment())?.commit()
                    } else {
                        resetirajIgru()
                        pokreniIgru()
                    }
