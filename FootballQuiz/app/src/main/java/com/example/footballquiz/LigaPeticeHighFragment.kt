...
class LigaPeticeHighFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_ligapetice_high, container, false)
    }
            ...
            query.addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    for (snapshot in dataSnapshot.children) {
                        val trenutniScore = snapshot.child("liga_petice_score").value as? Long
                        val trenutniHighScore = snapshot.child("liga_petice_highScore").value as? Long
    ...
    private fun getQuestion(questionNumber: Int) {
        pitanjaRef.child("Liga petice").child("Teška kategorija").child("$questionNumber")
        ...
        if (!isFragmentDestroyed) {
            try {
                ...
                builder.setPositiveButton("Sljedeće pitanje") { dialog, which ->
                    ...
                    brojPitanja++
                    if(brojPitanja > 3){
                        fragmentManager?.beginTransaction()?.replace(R.id.fragmentContainer, LigaPeticeHighscoreFragment())?.commit()
                    }
                    else {
                        resetirajIgru()
                        pokreniIgru()
